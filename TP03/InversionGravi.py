...
def grav_cylindre(x, rho, r, p_x, p_z, G=6.674e-8):
    return old_div((old_div((2 * np.pi * G * rho * r**2), p_z) ), (1 + ((old_div((p_x - x),p_z))**2)))

def grav_sphere(x, rho, r, p_x, p_z, G=6.674e-8):
    return (old_div(4.,3))*np.pi*G*rho*p_z*r**3 / ( ((p_x - x)**2 + p_z**2)**(old_div(3.,2)) )
...
class MainApplication(object):
    ...
    def plot_rms_progress(self):
        ...
        self.rms_line.set_xdata(list(range(len(self.rms_list))))
    ...
    def MC_inversion(self):
        ...
        for i in range(self.iterations.get()):
            if i % (old_div(self.iterations.get(),100)) == 0:
    ...
    def plot_data(self):
        ...
        ax_data.plot(old_div(axeX,100), 1000*self.g, 'k.')
        self.reponse, = ax_data.plot(old_div(axeX,100), np.zeros(len(self.axeX)), 'r-')
